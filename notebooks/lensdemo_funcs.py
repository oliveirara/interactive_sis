...
def xy_rotate(x, y, xcen, ycen, phi):
    ...
    """
    xnew = (x - xcen) * N.cos(phi) + (y - ycen) * N.sin(phi)
    ynew = (y - ycen) * N.cos(phi) - (x - xcen) * N.sin(phi)
    return (xnew, ynew)


def gauss_2d(x, y, par, e_s):
    """
    NAME: gauss_2d
    ...
    r_ell_sq = (
        (xnew ** 2) * (1 - e_s) / par[0] ** 2 + (ynew ** 2) * (1 + e_s) / par[0] ** 2
    ) / N.abs(par[1]) ** 2
...
def sie_grad(x, y, par):
    ...
    phiq = 0.0 if (len(par) < 5) else N.rad2deg(par[4])
